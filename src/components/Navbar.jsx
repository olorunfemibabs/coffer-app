  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setProvider(web3auth.provider ?? "https://eth-sepolia.g.alchemy.com/v2/PN7ox_cWivKpFnRnE5YKoJ5Vyp8-Bx5j");
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-[#010101] text-base font-medium">
        ...
        <ul
          className={`md:flex bg-white md:bg-transparent md:items-center md:pb-0 pb-12 absolute md:static  md:z-auto z-[100] right-4  w-[70%] md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in-out ${open ? "top-20 rounded-2xl shadow-lg" : "top-[-490px]"
            }`}
        >
          {navLinks.map((link) => (
            <li
            ...
            >
              <Link
                ...
                className={` border border-transparent ${router.pathname == link.address
